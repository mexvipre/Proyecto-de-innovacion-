...
if (isset($input["imagen"]) && isset($input["nombre_imagen"]) && isset($input["iddetequipo"])) {
    ...
    try {
        ...
        $objeto = $bucket->upload(fopen($rutaTemporal, 'r'), [
            'name' => "evidencia_entrada/" . $nombreArchivo
        ]);
    
        $objeto->update([], ['predefinedAcl' => 'PUBLICREAD']);
        $url = "https://storage.googleapis.com/$bucketName/evidencia_entrada/$nombreArchivo";
    
        // Guardar la URL en evidencias_entrada
        $stmt = $pdo->prepare("INSERT INTO evidencias_entrada (ruta_Evidencia_Entrada) VALUES (:ruta)");
        $stmt->execute(['ruta' => $url]);
    
        $idEvidencia = $pdo->lastInsertId();
    
        // Vincular la evidencia al equipo
        $update = $pdo->prepare("UPDATE detequipos SET idEvidencia = :idEvidencia WHERE iddetequipo = :iddetequipo");
        $update->execute([
